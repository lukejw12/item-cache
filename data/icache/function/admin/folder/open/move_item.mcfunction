$execute unless data storage icache:cache folders.$(folder_name) run tellraw @s [{"text":"[iCache]","color":"gold"},{"text":" Folder '$(folder_name)' does not exist.","color":"red"}]
$execute unless data storage icache:cache folders.$(folder_name) run return 0
